...
class ProfilePage extends React.Component {
    ...
    async componentDidMount() {
        if(!this.props.user.role) window.location.replace("https://localhost:3000/unauthorized");

        axios
            .get('https://localhost:8443/api/users/isAccountApproved/' + this.props.user.id,
                {  headers: {
    ...
    async sendData(){
        axios
            .post('https://localhost:8443/api/users/approveAccount', {
